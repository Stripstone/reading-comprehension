...
window.setPlaybackRate = function setPlaybackRate(value) {
  const rate = Math.max(0.5, Math.min(2, parseFloat(value) || 1));
  TTS_STATE.rate = rate;
  try { localStorage.setItem('rc_tts_speed', String(rate)); } catch (_) {}
  try { TTS_AUDIO_ELEMENT.playbackRate = rate; } catch (_) {}
  try { if (TTS_STATE.audio) TTS_STATE.audio.playbackRate = rate; } catch (_) {}
  return rate;
};

window.getPlaybackStatus = function getPlaybackStatus() {
  const cloudPlaying = !!(TTS_STATE.audio && !TTS_STATE.audio.paused);
  const browserPlaying = !!(browserTtsSupported() && window.speechSynthesis.speaking && !window.speechSynthesis.paused);
  const paused = !!((TTS_STATE.audio && TTS_STATE.audio.paused) || (browserTtsSupported() && window.speechSynthesis.paused));
  return { active: !!TTS_STATE.activeKey, playing: cloudPlaying || browserPlaying, paused };
};

window.getAutoplayStatus = function getAutoplayStatus() {
  return { enabled: !!AUTOPLAY_STATE.enabled };
};

window.getCountdownStatus = function getCountdownStatus() {
  return { pageIndex: AUTOPLAY_STATE.countdownPageIndex, seconds: AUTOPLAY_STATE.countdownSec, active: AUTOPLAY_STATE.countdownPageIndex !== -1 && AUTOPLAY_STATE.countdownSec > 0 };
};

window.pauseOrResumeReading = function pauseOrResumeReading() {
  const status = window.getPlaybackStatus();
  if (!status.active) return status;
  if (status.playing) ttsPause();
  else ttsResume();
  return window.getPlaybackStatus();
};

window.toggleAutoplay = function toggleAutoplay() {
  AUTOPLAY_STATE.enabled = !AUTOPLAY_STATE.enabled;
  if (!AUTOPLAY_STATE.enabled) ttsAutoplayCancelCountdown();
  return window.getAutoplayStatus();
};
