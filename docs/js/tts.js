...
function setPlaybackRate(rate) {
  const n = Number(rate);
  const safeRate = Number.isFinite(n) ? Math.max(0.5, Math.min(2, n)) : 1;
  try {
    localStorage.setItem('rc_tts_speed', String(safeRate));
  } catch (_) {}
  TTS_STATE.rate = safeRate;
  try {
...
function pauseOrResumeReading() {
  ...
  const idx = (typeof lastFocusedPageIndex === 'number' && lastFocusedPageIndex >= 0) ? lastFocusedPageIndex : 0;
  const text = Array.isArray(pages) ? String(pages[idx] || '') : '';
  if (text) {
    try { ttsSpeakQueue(`page-${idx}`, [text]); } catch (_) {}
    return 'playing';
  }
  return 'idle';
}
...
function getPlaybackStatus() {
  return {
    activeKey: TTS_STATE.activeKey,
    isPlaying: !!TTS_STATE.activeKey,
    pageIndex: (typeof lastFocusedPageIndex === 'number' && lastFocusedPageIndex >= 0) ? lastFocusedPageIndex : 0,
    rate: Number(TTS_STATE.rate || localStorage.getItem('rc_tts_speed') || 1) || 1
  };
}

function getAutoplayStatus() {
  return {
    enabled: !!AUTOPLAY_STATE.enabled,
    countdownPageIndex: AUTOPLAY_STATE.countdownPageIndex,
    countdownSec: AUTOPLAY_STATE.countdownSec
  };
}

function getCountdownStatus() {
  return {
    pageIndex: AUTOPLAY_STATE.countdownPageIndex,
    secondsRemaining: AUTOPLAY_STATE.countdownSec,
    active: AUTOPLAY_STATE.countdownPageIndex !== -1 && AUTOPLAY_STATE.countdownSec > 0
  };
}
