...
function ttsBlockSnapshot() {
  ...
  const pageIdx = key && /^page-(\d+)$/.test(key) ? parseInt(key.slice(5), 10) : -1;
  return {
...
function browserSpeakQueue(key, parts) {
  ...
  if (window.speechSynthesis.speaking || window.speechSynthesis.pending) ttsStop();
...
function browserSpeakPageFromSentence(key, blockIdx) {
  ...
  TTS_STATE.browserSpeakFromBlock(target);
  return true;
}
...
function ttsPause() {
  ...
  TTS_STATE.pausedBlockIndex = TTS_STATE.activeBlockIndex;
  ...
  ttsDiagPush('paused', {
    ...
    strategy: TTS_DEBUG.lastPauseStrategy,
    preservedBlock: TTS_STATE.pausedBlockIndex,
    preservedKey: TTS_STATE.pausedPageKey,
  });
}
...
function ttsResume() {
  ...
  if (TTS_STATE.audio && TTS_STATE.audio.paused) {
    ...
    ttsDiagPush('resumed', { before, after: ttsBlockSnapshot(), route: 'cloud-audio-resume', preservedBlock: before.pausedBlockIndex });
    return;
  }
  ...
  if (browserTtsSupported() && TTS_STATE.browserPaused) {
    ...
    const key = TTS_STATE.pausedPageKey || TTS_STATE.activeKey;
    const blockIdx = TTS_STATE.pausedBlockIndex >= 0 ? TTS_STATE.pausedBlockIndex : 0;
    const ok = browserSpeakPageFromSentence(key, blockIdx);
    TTS_STATE.pausedBlockIndex = -1;
    TTS_STATE.pausedPageKey = null;
    ttsDiagPush('resumed', {
      before, after: ttsBlockSnapshot(),
      route: ok ? 'browser-restart-from-block' : 'browser-restart-failed',
  ...
  ttsDiagPush('resumed', { before, after: ttsBlockSnapshot(), route: 'browser-native-resume' });
}
...
function pauseOrResumeReading() {
  ...
  if (before.playback.paused) {
    ttsResume(); route = 'resume'; outcome = 'resumed';
  } else {
    ttsPause(); route = 'pause'; outcome = 'paused';
  }
...
async function ttsSpeakQueue(key, parts) {
  ...
  try {
    ...
    if (optsForKeySentenceMarks(key)) {
      const pageIndex = parseInt(String(key).slice(5), 10);
      if (Number.isFinite(pageIndex)) { ttsKeepWarmForAutoplay(); ttsAutoplayScheduleNext(pageIndex); }
    }
...
function ttsJumpSentence(delta) {
  ...
  const pageMatch = String(key).match(/^page-(\d+)$/);
  const sourcePage = pageMatch ? parseInt(pageMatch[1], 10) : -1;
  ...
  if (audio && marks && blockCount > 0) {
    ...
    if (target >= blockCount) { // next at last block → cross to next page
      const moved = ttsJumpPage(1);
      const skipResult = { at: new Date().toISOString(), type: 'block', delta, sourcePage, sourceBlock, resolvedPage: sourcePage + 1, resolvedBlock: 0, crossPage: true, moved, path: 'cloud-cross-page', clippingProtection: false };
  ...
  if (browserTtsSupported() && TTS_STATE.browserSpeakFromBlock) {
    ...
    if (target >= rangeCount) {
      const moved = ttsJumpPage(1);
      const skipResult = { at: new Date().toISOString(), type: 'block', delta, sourcePage, sourceBlock, resolvedPage: sourcePage + 1, resolvedBlock: 0, crossPage: true, moved, path: 'browser-cross-page', clippingProtection: false };
      TTS_DEBUG.lastSkip = skipResult;
      ttsDiagPush('skip-block', skipResult);
      return moved;
    }
...
function restartLastSpokenPageTts() {
  ...
  // lastPageKey is now reliably set whenever a page key is activated.
