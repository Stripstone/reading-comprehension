...
// ===================================
  ...
  // ===================================
  // 📘 How This Works (Instructions Modal) + ❤️ Support Banner
  // ===================================

  (function initHowItWorksModalAndSupportBanner() {
    ...
    // Support banner (delayed + interaction gated)
    const supportBanner = document.getElementById('supportBanner');
    const supportCloseBtn = document.getElementById('supportCloseBtn');

    // Show only once per page-load. Closing docks it at the bottom of the page.
    let bannerDismissed = false;
    let bannerShown = false;

    // Delay between 300–600s (randomized per page load)
    const bannerDelayMs = 300000 + Math.floor(Math.random() * 300000);
    // A few interactions (clicks/typing) before we consider showing it
    const interactionThreshold = 5;
    let interactions = 0;
    let delayElapsed = false;

    function maybeShowSupportBanner() {
      if (!supportBanner) return;
      if (bannerDismissed || bannerShown) return;
      if (!delayElapsed) return;
      if (interactions < interactionThreshold) return;

      bannerShown = true;
      supportBanner.classList.remove('is-hidden');
      supportBanner.classList.remove('docked');
    ...
    // Count interactions (lightweight)
    function bumpInteraction() {
      interactions += 1;
      if (interactions >= interactionThreshold) {
        maybeShowSupportBanner();
      }
    }

    document.addEventListener('click', bumpInteraction, { capture: true, passive: true });
    document.addEventListener('keydown', (e) => {
      if (e.key && e.key.length === 1) bumpInteraction();
    }, { capture: true });

    // Close button: dock banner to bottom of page (in-flow) and prevent re-show until reload
    function dockSupportBanner() {
      if (!supportBanner) return;
      bannerDismissed = true;
      supportBanner.classList.remove('is-hidden');
      supportBanner.classList.add('docked');
      document.body.classList.remove('support-banner-visible');
    }

    if (supportCloseBtn) supportCloseBtn.addEventListener('click', dockSupportBanner);
