/* Extracted from docs/index.html to keep shell DOM-centered. */
// ============================================================
    ...
    function scrollReadingPageTo(index) {
        try {
            const pageEls = Array.from(document.querySelectorAll('.page'));
            if (!pageEls.length) return false;
            const nextIndex = Math.max(0, Math.min(pageEls.length - 1, Number(index) || 0));
            const target = pageEls[nextIndex];
            if (!target) return false;
            document.querySelectorAll('.page-active').forEach((el) => el.classList.remove('page-active'));
            target.classList.add('page-active');
            target.scrollIntoView({ behavior: 'smooth', block: 'start' });
            try { currentPageIndex = nextIndex; } catch (_) {}
            try { lastFocusedPageIndex = nextIndex; } catch (_) {}
            setTimeout(() => target.classList.remove('page-active'), 600);
            try { if (typeof schedulePersistSession === 'function') schedulePersistSession(); } catch (_) {}
            try { if (typeof updateShellPageProgress === 'function') updateShellPageProgress(); } catch (_) {}
    ...
    function handleReadingPrev() {
        try {
            const total = document.querySelectorAll('.page').length || 0;
            if (!total) return false;
            const current = (typeof getCurrentReadingPageIndex === 'function') ? getCurrentReadingPageIndex() : 0;
            const target = (Number(current) <= 0) ? (total - 1) : (Number(current) - 1);
            return scrollReadingPageTo(target);
        } catch (_) {
            return false;
        }
    }
    ...
    function handleReadingNext() {
        try {
            ...
            if (!total) return false;
            const current = (typeof getCurrentReadingPageIndex === 'function') ? getCurrentReadingPageIndex() : 0;
            const target = (Number(current) + 1) % total;
            return scrollReadingPageTo(target);
        } catch (_) {
            return false;
        }
    ...
    function syncShellPlaybackControls() {
        ...
        if (btn && typeof getPlaybackStatus === 'function') {
            ...
            btn.innerHTML = label === 'Pause'
                ...
                : '<svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg> ' + label;
            try {
                const support = (typeof getTtsSupportStatus === 'function') ? getTtsSupportStatus() : null;
                const blocked = !!(support && !support.playable && !status.active);
                btn.disabled = blocked;
                btn.setAttribute('aria-disabled', String(!!btn.disabled));
                [prevBtn, nextBtn].forEach((navBtn) => {
                    if (!navBtn) return;
                    const pageCount = document.querySelectorAll('.page').length || 0;
                    navBtn.disabled = pageCount <= 1;
                    navBtn.setAttribute('aria-disabled', String(!!navBtn.disabled));
                });
    ...
    // Autoplay button — runtime-owned, shell only reflects
    function handleAutoplayToggle() {
        const checkbox = document.getElementById('autoplayToggle');
        const btn      = document.getElementById('shell-autoplay-btn');
        let enabled = false;
        try { if (typeof toggleAutoplay === 'function') enabled = !!toggleAutoplay(); } catch(_) {}
    ...
    window.getShellDiagnosticsSnapshot = function getShellDiagnosticsSnapshot() {
        ...
        return {
            ...
            controls: {
                ...
                prev: snapshotShellControl('#shell-prev-btn'),
                play: snapshotShellControl('#shell-pause-btn'),
                next: snapshotShellControl('#shell-next-btn'),
                autoplay: snapshotShellControl('#autoplayToggle')
            },
            pageReadButtons: {
