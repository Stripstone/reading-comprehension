...
## 19. Build an apply-safe patch file

Prefer a Git-produced patch whenever possible.

Best option:

```bat
cd C:\Users\Triston Barker\Documents\GitHub\reading-comprehension\
git diff -- docs/js/library.js > patch.diff
```

What it does:
- creates a real unified diff that `git apply` can read
- avoids malformed hunk counts or missing context lines
- is safer than hand-writing patch headers

If you must draft a patch manually, the file must look like a real unified diff:
...
Rules:
- do not wrap the saved patch file in markdown fences
- every changed section needs both file headers: `--- a/...` and `+++ b/...`
- every hunk needs a valid `@@ -old,start +new,start @@` header
- context lines start with a single space
- removed lines start with `-`
- added lines start with `+`
- keep the path exactly as it exists in the repo
...
- rebuild the patch from the exact current file, not from memory
- check that the hunk line counts and context match the live file
- prefer exporting a new patch from Git instead of editing the patch by hand

Helpful fallback when you need Git to build the patch for you from two files:

```bat
git diff --no-index old-library.js new-library.js > patch.diff
```

Then rename the paths inside the patch if needed so they match the repo target before applying.



## 12. Build an apply-safe patch file

Prefer a Git-generated diff over a hand-written patch whenever possible.

### Best option: export from the repo
...
```bat
git diff -- docs/js/state.js docs/js/library.js docs/js/tts.js > patch.diff
```

### If you must build a patch from two files

Use Git to compare them instead of hand-writing hunk headers:
...
- start with both lines:
  - `--- a/path/to/file`
  - `+++ b/path/to/file`
- each hunk must start with a valid `@@ -oldStart,oldCount +newStart,newCount @@` header
- context line counts in the hunk header must match the actual hunk body
- keep unchanged context lines around edits
- end the file with a final newline
- wrap the patch in triple backticks only when pasting it into chat, not inside the saved `.diff` file
...
### If `git apply --check` says `corrupt patch`

That usually means one of these:
- the hunk header counts are wrong
- the patch is missing enough context lines
- the file path in the patch does not match the repo path
- the patch file was copied with broken formatting
- the patch file does not end with a newline
